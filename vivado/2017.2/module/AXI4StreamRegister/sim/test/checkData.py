...
fin = open('../../AXI4StreamRegister/AXI4StreamRegister.sim/sim_1/behav/result.txt', 'r')
fref = open('ref.txt', 'r')
line_no = 1
lin = fin.readline()
ok = True
while lin:
    ...
    if (tlast != ref_tlast):
        print("line {0}: Invalid tlast {1:04x}, expected {2:04x}".
              format(line_no, tlast, ref_tlast))
        ok = False
